...
import { CHILDREN_CHECK, CHILDREN_FETCH, fetchChildren, setChildren } from "../features/ChildrenSlice";
...
export function fetchChildrenEpic(
  ...
  return action$.pipe(
    ...
    switchMap((action) => {
      const request = new ChildrenService().fetchChildren("11fc220c-ebba-4e55-9346-cd1eed714620", "fb6c8114-387e-4051-8cf7-4e388a77b673").pipe(
        map((resp) => {
...
export function checkChildEpic(
  ...
  return action$.pipe(
    ...
    switchMap((action) => {
        const request = new ChildrenService().checkChild(action.payload.childId, action.payload.time, action.payload.action).pipe(
          map((resp) => {
            // return updateChild(action.payload)
